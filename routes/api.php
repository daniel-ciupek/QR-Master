...
use App\Http\Controllers\Scim\ScimConfigController;
...
Route::prefix('scim/v2/{teamSlug}')->name('scim.')->middleware(['scim.auth', 'throttle:scim'])->group(function () {
    ...
    Route::get('Users/{userId}', [ScimUserController::class, 'show'])->name('users.show');
    Route::put('Users/{userId}', [ScimUserController::class, 'update'])->name('users.update');
    Route::patch('Users/{userId}', [ScimUserController::class, 'update'])->name('users.patch');
    Route::delete('Users/{userId}', [ScimUserController::class, 'destroy'])->name('users.destroy');
});
